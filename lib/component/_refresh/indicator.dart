...
double kMaterialHorizontalFrictionFactor(double overscrollFraction) => 1.0 * math.pow(1 - overscrollFraction, 2);
...
class _AppIndicatorState extends State<AppIndicator> {
  ...
  double? get _value {
    ...
    if (_mode == IndicatorMode.drag || _mode == IndicatorMode.armed) {
      ...
      return math.min((_offset - noneOffset) / (_actualTriggerOffset * 1.25 - noneOffset), 1) * 0.75;
    }
    return null;
  }
  ...
  Color? get _color {
    ...
    final color = widget.color ?? ProgressIndicatorTheme.of(context).color ?? Theme.of(context).colorScheme.primary;
  ...
  Widget _buildIndicator() {
    ...
    return Container(
      alignment: _axis == Axis.vertical ? (widget.reverse ? Alignment.topCenter : Alignment.bottomCenter) : (widget.reverse ? Alignment.centerLeft : Alignment.centerRight),
      ...
      child: Stack(
        ...
        children: [
          ...
          if (_mode == IndicatorMode.inactive && _result == IndicatorResult.noMore) widget.noMoreIcon ?? const Icon(Icons.inbox_outlined),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.state.indicator.infiniteOffset != null && widget.state.indicator.position == IndicatorPosition.locator && (_mode != IndicatorMode.inactive || _result == IndicatorResult.noMore)) {
